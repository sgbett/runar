...
\begin{lstlisting}[style=tsop,language=C,
...
\midrule
\texttt{.tsop.ts} (or default) & TypeScript (via ts-morph) & TS, Go, Rust \\
\texttt{.tsop.sol} & Solidity-like (recursive descent) & TS, Go, Rust \\
\texttt{.tsop.move} & Move-style (recursive descent) & TS, Go, Rust \\
\texttt{.tsop.go} & Go (\texttt{go/parser} + AST mapping) & Go only \\
\texttt{.tsop.rs} & Rust (tokenizer + recursive descent) & Rust only \\
\bottomrule
\end{tabular}
\end{center}

The TypeScript reference compiler dispatches \texttt{.tsop.sol} and
\texttt{.tsop.move} to their respective hand-written parsers, with all
other extensions falling through to the TypeScript parser. The
\texttt{.tsop.go} and \texttt{.tsop.rs} formats are handled exclusively
...
Unlike many compilers, \tsop{} does \emph{not} desugar logical operators
...
operands are always evaluated. This is safe because \tsop{} programs are
...
\tsop{} includes experimental support for post-quantum signature schemes
...
\tsop{} supports all six SHA-256 parameter sets (128s/f, 192s/f, 256s/f)
...
This section presents empirical measurements of the \tsop{} compiler across
...
The \tsop{} test suite comprises three tiers of validation:
...
\midrule
TypeScript (\texttt{.tsop.ts}) & 12 & TS, Go, Rust & Stable \\
Solidity-like (\texttt{.tsop.sol}) & 9 & TS, Go, Rust & Experimental \\
Move-style (\texttt{.tsop.move}) & 9 & TS, Go, Rust & Experimental \\
Go (\texttt{.tsop.go}) & 10 & Go & Experimental \\
Rust (\texttt{.tsop.rs}) & 10 & Rust & Experimental \\
\midrule
...
\caption{Compiled script sizes for the example contract suite.
  All contracts except PostQuantumWallet produce byte-identical output
  across the TS and Go compilers.}
\label{tab:sizes}
\end{table}
...
The Go frontend (\texttt{.tsop.go}) is supported only by the Go compiler,
and the Rust frontend (\texttt{.tsop.rs}) only by the Rust compiler.
...
the compiled output without changes to the \tsop{} compiler.
...
test corpus to achieve stronger coverage guarantees, completing the SLH-DSA
Bitcoin Script codegen, and extending cross-compiler conformance to the Go
and Rust frontend formats.
