...
const source = readFileSync(join(__dirname, 'FunctionPatterns.runar.sol'), 'utf8');
const FILE_NAME = 'FunctionPatterns.runar.sol';

const OWNER_PK = ALICE.pubKey;
const OWNER_SIG = signTestMessage(ALICE.privKey);

function make(balance = 10000n) {
  return TestContract.fromSource(source, { owner: OWNER_PK, balance }, FILE_NAME);
}

describe('FunctionPatterns (Solidity)', () => {
